 ...
 */
public class xBinomialModel2016 extends AbstractOptionClass2017 implements DerivativesCalc {
    
    private int UpBound;
    private double   interv, sqrtInterv;
    private double u, a, d, p, pp,z;
    private double AssetAtNode,BtreeEsp;
    private double vtheta, delt1, delt2, UVu4, UVd4, UVu2d2;   
    protected int steps,mult;
    
    
    xBinomialModel2016(){}
    public xBinomialModel2016(char tipoEjercicio,Underlying Und, char callPut, double strike,double daysToExpiration,double tasa,double impliedVol,double optionMktValue,int steps){
        
        
         pModelName              ="BinomialModel ver2";
        this.modelNumber        =3; 
	...
	//European o Americam
		...
		for (int j = 0; j <= (UpBound - i); j++){
			AssetAtNode = underlyingNPV*Math.pow(u, (UpBound - i - j))*Math.pow(d, j);
	...
	for (int i = (UpBound - 7); i <= UpBound; i++){
		for (int j = 0; j <= (UpBound - i); j++){
			AssetAtNode = underlyingNPV*Math.pow(u, (UpBound+1 - i - j))*Math.pow(d, j);
			BtreeEsp = (p*vPrices[j] + pp*vPrices[j + 1])*z;
			vPrices[j] = Math.max((AssetAtNode-strike)*mult*multCallPut, BtreeEsp);
		}
